...
run_test "reviewer fixture: resolved YAML" bash -c 'out=$("$1" workflow apply "$2" -o yaml) && grep -q "source: REVIEW.md" <<<"$out" && grep -q "source: fixtures/pr.diff" <<<"$out" && grep -q "type: claude" <<<"$out"' _ "$HARNESS" "$ROOT/workloads/github-pr-reviewer/workflow/harness.yaml"
...
if $LIVE && "$CLI" inference get >/dev/null 2>&1; then
  ...
  run_test "live: delete" "$CLI" sandbox delete suite-sdk-live
else
