...
# Same lifecycle pattern as test/vertex-gemini-opencode.sh; no shared runner API.
cleanup() {
  local status=$?
  trap - EXIT INT TERM
  if [[ -n "$apply_pid" ]]; then
  ...
  if $created_workspace; then
    timeout 30s ./harness delete --gateway "$gateway" --workspace "$workspace" --sandboxes || status=1
...
wait "$apply_pid"
apply_pid=""
# Ignore harness status lines, but reject errors, tools, and missing/truncated completion.
jq -Rse 'split("\n") | map(fromjson?) |
