#!/usr/bin/env bash
# Trusted local CI provisioning only. The review CLI uses an existing target.
set -euo pipefail
umask 077
cd "$(dirname "$0")/.."
...
cleanup() {
  ...
  # Record provisioning cleanup separately from task execution. The CLI's
  # execution.json/summary describe its sandbox run, not this local setup.
  if [[ -d "${REVIEW_DIR:-}" ]]; then
    printf '{"exitCode":%d}\n' "$status" > "$REVIEW_DIR/local-setup.json"
  fi
  exit "$status"
}
trap cleanup EXIT
...
./harness github review run "$@" --gateway "$gateway" --workspace "$workspace" &
review_pid=$!
set +e
wait "$review_pid"
status=$?
