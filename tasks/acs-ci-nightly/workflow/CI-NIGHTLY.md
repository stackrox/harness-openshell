...
4. Find failures from the configured lookback window in the Prow nightly jobs
   ...
   the existing 24-hour triage window. Use `gcloud storage` to discover
   nightly `<build>/finished.json` objects, then enumerate exact job/build
   prefixes and read the root result object. Do not infer a job's result from
   a `latest-build.txt` marker.
